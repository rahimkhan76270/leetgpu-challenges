...
def load_cuda_lib():
    ...
    lib_file = Path("starter/libvector_add.so")
...
def test_vector_add():
    ...
    def run_test(test_case):
        A = test_case["A"]
        B = test_case["B"]
        C_cuda = test_case["C"].clone()  # Clone to keep original tensor unchanged
        C_torch = test_case["C"].clone()
        N = test_case["N"]

        # Run reference implementation
        challenge.reference_impl(A, B, C_torch, N)
        ...
        C_ptr = ctypes.cast(C_cuda.data_ptr(), ctypes.POINTER(ctypes.c_float))
